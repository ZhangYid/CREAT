...
def process_data(g, 
    ...
    '''add eids of case pairs'''
    # mAP_eid, sAP_eid = (torch.tensor([32163]), torch.tensor([32164]))
    # test_pos_eids = torch.cat((test_pos_eids, mAP_eid))
    # test_pos_eids = torch.cat((test_pos_eids, sAP_eid))
    # if mAP_eid not in test_pos_eids:
    #     test_pos_eids = torch.cat((test_pos_eids, mAP_eid))
    # if sAP_eid not in test_pos_eids:
    #     test_pos_eids = torch.cat((test_pos_eids, sAP_eid))   
    #
    # Asthma_eid, T2MD_eid = (torch.tensor([5256]), torch.tensor([6519]))
    # if Asthma_eid not in test_pos_eids:
    #     test_pos_eids = torch.cat((test_pos_eids, Asthma_eid))
    # if T2MD_eid not in test_pos_eids:
    #     test_pos_eids = torch.cat((test_pos_eids, T2MD_eid)) 
    ''''''
    # query_src_nids, query_tgt_nids = torch.load('/home/lab/zyd/AP_query_nids')
    train_pos_u, train_pos_v = u[train_pos_eids], v[train_pos_eids]
    # train_pos_u, train_pos_v = torch.cat((u[train_pos_eids],query_src_nids)), torch.cat((v[train_pos_eids],query_tgt_nids))
    val_pos_u, val_pos_v = u[val_pos_eids], v[val_pos_eids]
    test_pos_u, test_pos_v = u[test_pos_eids], v[test_pos_eids]
    # test_pos_u, test_pos_v = torch.cat((u[test_pos_eids],query_src_nids)), torch.cat((v[test_pos_eids],query_tgt_nids))
    ...
    #     test_pos_u, test_pos_v = gt_edges
    #     m = test_pos_u.shape[0]
    #     test_neg_u, test_neg_v = test_neg_u[:m], test_neg_v[:m]
...
def load_dataset(dataset_dir, dataset_name, pred_etype, val_ratio, test_ratio, eval_exp=False):
    '''
    Parameters
    ----------
    dataset_dir : string
    ...
    g = graph_list[0]
    if 'wHERB' in dataset_name:
        src_ntype, tgt_ntype, pred_etype = 'herb', 'disease', 'has-effects-on'
    else:
        src_ntype, tgt_ntype, pred_etype = 'drug', 'disease', 'treats'

    
    if pred_etype == 'treats':
        src_ntype, tgt_ntype = 'drug', 'disease'
    elif pred_etype == 'has-effects-on':
        src_ntype , tgt_ntype = 'herb' , 'disease' 
    else:
        raise ValueError('Unknow prediction argument')
