...
def get_ntype_pairs_to_cannonical_etypes(ghetero, pred_etype='has-effects-on'):
    '''
...
def hetero_src_tgt_khop_in_subgraph(src_ntype, src_nid, tgt_ntype, tgt_nid, ghetero, k):
    ...
    sghetero_feat_eid: Tensor
        The original `ghetero` edge ids of subgraph edges, for feature identification
    '''
...
def get_comp_g_edge_labels(comp_g, edge_labels):
    ...
    for can_etype in edge_labels:
        ...
        # start_included_nid_mask = (start_nids <= start_comp_g_max_nid) and (start_nids in ntypes_to_nids[start_ntype])
        # end_included_nid_mask = (end_nids <= end_comp_g_max_nid) and (end_nids in ntypes_to_nids[end_ntype])
        # comp_g_included_nid_mask = torch.tensor([end_included_nid_mask & start_included_nid_mask])
        ...
        try:
            ...
            Error = 'edges in label but not in comp_g'       
...
def get_comp_g_path_labels(comp_g, path_labels):
    ...
    for path in path_labels:
        ...
        for can_etype, start_nid, end_nid in path:
            ...
            # For edges in label but not in comp_g, exclude them
            start_included_nid_mask = start_nid <= start_comp_g_max_nid
            end_included_nid_mask = end_nid <= end_comp_g_max_nid
            comp_g_included_nid_mask = end_included_nid_mask & start_included_nid_mask
        
            if comp_g_included_nid_mask:
                ...
                try:
                    ...
                    Error = 'edges in labe not in comp_g'
...
def eval_edge_mask_auc(edge_mask_dict, edge_labels):
    ...
    pred_label = np.where(y_score >= 0.5, 1, 0)
    recall = recall_score(y_true, pred_label)
    cm = normalize(confusion_matrix(y_true, pred_label), norm='l1')

    try:
        roc = roc_auc_score(y_true, y_score)
    except ValueError:
        roc = -1
    return roc, recall, cm
     ...
     # Start labelling nodes
        if label == 'none':
            ...
            if ghetero.ndata.get(label):
                homo_nids_to_hetero_ndata_feat = get_homo_nids_to_hetero_ntype_data_feat(ghetero, label, node_name_dict)
                if label_offset:
...
def get_homo_nids_to_hetero_ntype_data_feat(ghetero, feat=dgl.NID, node_name_dict=None):
    '''
    ...
    if node_name_dict:
        for ntype in ghetero.ntypes:   
            hetero_ndata_feat += [f'{node_name_dict[ntype][feat]}' for feat in ghetero.ndata[feat][ntype].tolist()]
    else:
        ntype_short = {'disease':'dis', 'drug':'dg', 'target':'tg', 'pathway':'pw'}
        for ntype in ghetero.ntypes:   
            hetero_ndata_feat += [f'{ntype_short[ntype]}' + f'{feat}' for feat in ghetero.ndata[feat][ntype].tolist()]
            # hetero_ndata_feat += [f'{ntype}' + f'{feat}' for feat in ghetero.ndata[feat][ntype].tolist()]
